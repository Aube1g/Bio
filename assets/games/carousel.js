...
export function setupCarousel(games, { t, motionEnabled }) {
  ...
  const DEPTH_VARS = ['--rz', '--tz', '--ty', '--sc', '--dim', '--sat', '--bri'];
  function paint() {
    frame = 0;
    const index = nearest();
    ...
    if (!motionOff) {
      ...
      cards.forEach((card, i) => {
        ...
        card.style.setProperty('--sat', (1 - Math.min(a * 0.42, 0.6)).toFixed(3));
        card.style.setProperty('--bri', (1 - Math.min(a * 0.07, 0.14)).toFixed(3));
